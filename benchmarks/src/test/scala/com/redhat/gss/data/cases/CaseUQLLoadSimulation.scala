 ...
 * Time: 10:33 AM
 */
class CaseUQLLoadSimulation extends Simulation {
  val cases = Seq.fill(100)(Seq("00001002", "00001016", "00001024")).flatten
  ...
  val randomChain = for {
    i <- 1 to cases.length
      ...
      http("Case")
        .get(s"""/case?where=caseNumber is "${randCaseNumber}"""")
    ))

  val scn = scenario("Get a bunch of cases using UQL").randomSwitch(randomChain: _*)
